...
def edits(update, context:CallbackContext):
    reply_video = update.effective_message.reply_to_message.reply_text if update.effective_message.reply_to_message else update.effective_message.reply_text
    reply_video(random.choice(fun_strings.ANIME_EDITS)) 
...
EDITS_HANDLER = DisableAbleCommandHandler("edits", edits, run_async=True)
...
dispatcher.add_handler(EDITS_HANDLER)
